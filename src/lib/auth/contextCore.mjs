...
export async function resolveAuthenticatedContext({ user, portal, loadMembership }) {
  ...
  if (metadataRole === ROLES.PLATFORM_ADMIN) {
    if (portal !== "root") throw new AuthorizationError("PORTAL_FORBIDDEN", 403, "Esta cuenta solo puede acceder al Portal Root.");
  ...
  if (!membership || membership.status !== "active") {
    throw new AuthorizationError("MEMBERSHIP_INACTIVE", 403, "No existe una membresía cliente activa.");
  }
  if (![ROLES.COMPANY_ADMIN, ROLES.OPERATOR].includes(membership.role)) {
    throw new AuthorizationError("ROLE_FORBIDDEN", 403, "El rol de la membresía no está autorizado.");
  }
  if (portal !== "client") throw new AuthorizationError("PORTAL_FORBIDDEN", 403, "Esta cuenta solo puede acceder al Portal Cliente.");
  if (!membership.company || membership.company.status !== "active" || membership.company.relationship_type !== "client") {
    throw new AuthorizationError("COMPANY_INACTIVE", 403, "La empresa no está habilitada como cliente.");
  }
