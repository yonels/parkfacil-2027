...
export default function ParkingRatesManager({ parking, showCreateButton = true, openSignal }) {
  ...
  async function submit(event) {
    ...
    try {
      const response = await authenticatedFetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const body = await response.json();
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold text-[#041E42]">Tarifas operacionales</h1><p className="mt-1 text-sm text-slate-600">{parking.name} · minuto efectivo o tramo vencido, sin redondeo al alza.</p></div>{showCreateButton ? <button onClick={() => { setErrors({}); setRequestError(""); setForm({ ...initial }); }} className="inline-flex items-center gap-2 rounded-full bg-[#3150D8] px-4 py-2 text-sm font-semibold text-white"><Plus className="h-4 w-4" /> Nueva tarifa</button> : null}</div>
