...
import ParkingStructureAdmin from "./ParkingStructureAdmin";
...
export default function EstacionamientoDetalleAdmin({ parking, structure, company }) {
  ...
  return <AppShell title={parking.name} description="Administración del estacionamiento"><div className="space-y-6">
    <PageHeader title={parking.name} description={`${parking.code} · ${parking.city}`} actions={[
      ...
      <Link key="structure" href={`/estacionamientos/${parking.code}/${onStreet ? "sectores" : "niveles"}/nuevo`} className="inline-flex items-center gap-2 rounded-full bg-[#3150D8] px-4 py-2 text-sm font-semibold text-white"><Plus className="h-4 w-4" /> {onStreet ? "Crear sector" : "Crear nivel"}</Link>,
    ]} />
    ...
    <nav aria-label="Secciones del estacionamiento" className="flex gap-2 overflow-x-auto border-b border-slate-200 pb-3 text-sm font-semibold">
      <a href="#resumen" className="rounded-full bg-[#F5F9FF] px-4 py-2 text-[#3150D8]">Resumen</a>
      <a href="#estructura" className="rounded-full px-4 py-2 text-slate-600 hover:bg-slate-100">{onStreet ? "Sectores y Calles" : "Niveles y Zonas"}</a>
      {onStreet ? <a href="#operadores" className="rounded-full px-4 py-2 text-slate-600 hover:bg-slate-100">Operadores</a> : null}
      <a href="#infraestructura" className="rounded-full px-4 py-2 text-slate-600 hover:bg-slate-100">Infraestructura</a>
    </nav>

    <section id="resumen" className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <EstacionamientoResumen title="Capacidad total" value={metrics.capacity} description={onStreet ? "Calles activas" : "Zonas activas"} tone="info" />
        <EstacionamientoResumen title="Ocupadas" value={metrics.occupied} description="Unidades agregadas" tone="warning" />
        <EstacionamientoResumen title="Disponibles" value={metrics.available} description="Capacidad libre" tone="positive" />
        <EstacionamientoResumen title="Ocupación" value={`${metrics.occupancyPercentage}%`} description="Capacidad operacional" tone="neutral" />
      </div>
    ...
    <ParkingStructureAdmin parking={parking} structure={structure} />
    <section id="infraestructura" className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
