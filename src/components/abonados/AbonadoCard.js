...
export default function AbonadoCard({ abonado }) {
  ...
  return (
    <Link href={`/abonados/${abonado.id}`} className="group flex min-w-0 flex-col rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-[#3150D8] hover:shadow-md">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-[#3150D8]">{abonado.identificador}</p>
          <h3 className="mt-1 break-words text-lg font-semibold text-[#041E42]">{abonado.nombre}</h3>
        </div>
      ...
      <div className="mt-4 grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
        ...
        <div className="flex min-w-0 items-center gap-2"><Mail className="h-4 w-4 shrink-0 text-[#3150D8]" /><span className="truncate" title={abonado.correo}>{abonado.correo}</span></div>
