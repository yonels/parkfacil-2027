...
const transactions = [
  { id: "TRX-1008", company: "Clínica Ramis", date: "28/07/2026", time: "18:42", ticket: "TK-2208", plate: "PRVH63", parking: "Parking Centro", operator: "Carolina Muñoz", method: "Crédito", amount: 1680, status: "Conciliado" },
  { id: "TRX-1007", company: "Clínica Ramis", date: "28/07/2026", time: "18:16", ticket: "TK-2207", plate: "BHPR54", parking: "Parking Centro", operator: "Carolina Muñoz", method: "Crédito", amount: 1440, status: "Conciliado" },
  { id: "TRX-1006", company: "Inmobiliaria 5Q", date: "28/07/2026", time: "17:54", ticket: "TK-2206", plate: "DHGL81", parking: "Parking Norte", operator: "Felipe Soto", method: "Efectivo", amount: 1590, status: "Por conciliar" },
  { id: "TRX-1005", company: "Clínica Ramis", date: "28/07/2026", time: "16:31", ticket: "TK-2205", plate: "JRDR29", parking: "Parking Centro", operator: "Carolina Muñoz", method: "Crédito", amount: 2550, status: "Conciliado" },
  { id: "TRX-1004", company: "ParkFacil Operaciones", date: "28/07/2026", time: "15:27", ticket: "TK-2204", plate: "LXPB20", parking: "Parking Sur", operator: "Andrea Pérez", method: "Efectivo", amount: 480, status: "Observado" },
  { id: "TRX-1003", company: "Inmobiliaria 5Q", date: "28/07/2026", time: "13:08", ticket: "TK-2203", plate: "LPKB69", parking: "Parking Norte", operator: "Felipe Soto", method: "Crédito", amount: 1560, status: "Conciliado" },
  { id: "TRX-1002", company: "Clínica Ramis", date: "28/07/2026", time: "11:45", ticket: "TK-2202", plate: "SXLZ28", parking: "Parking Centro", operator: "Carolina Muñoz", method: "Efectivo", amount: 2910, status: "Por conciliar" },
  { id: "TRX-1001", company: "Clínica Ramis", date: "28/07/2026", time: "09:12", ticket: "TK-2201", plate: "HJCD22", parking: "Parking Centro", operator: "Carolina Muñoz", method: "Crédito", amount: 1740, status: "Conciliado" },
];

const closures = [
  { id: "CIE-041", company: "Clínica Ramis", parking: "Parking Centro", shift: "Turno mañana", operator: "Carolina Muñoz", expected: 842300, declared: 842300, difference: 0, status: "Cerrado" },
  { id: "CIE-042", company: "Inmobiliaria 5Q", parking: "Parking Norte", shift: "Turno mañana", operator: "Felipe Soto", expected: 426700, declared: 425200, difference: -1500, status: "Observado" },
  { id: "CIE-043", company: "ParkFacil Operaciones", parking: "Parking Sur", shift: "Turno tarde", operator: "Andrea Pérez", expected: 318400, declared: 0, difference: 0, status: "Pendiente" },
];

const dailyRevenueBase = [
...
const companyProfiles = {
  Todas: { factor: 1, total: 2478180, cash: 545670, cards: 1932510, months: [2054000, 2268000, 2478180] },
  "Clínica Ramis": { factor: 0.51, total: 1263872, cash: 240136, cards: 1023736, months: [1014200, 1146800, 1263872] },
  "Inmobiliaria 5Q": { factor: 0.3, total: 743454, cash: 223036, cards: 520418, months: [612500, 684300, 743454] },
  "ParkFacil Operaciones": { factor: 0.19, total: 470854, cash: 82500, cards: 388354, months: [427300, 436900, 470854] },
};
...
export default function RecaudacionPage() {
  ...
  const companyTransactions = useMemo(
    () => transactions.filter((item) => company === "Todas" || item.company === company),
    [company],
  );
  const companyClosures = useMemo(
    () => closures.filter((item) => company === "Todas" || item.company === company),
    [company],
  );
  const dailyRevenue = useMemo(
    () => dailyRevenueBase.map((item) => ({ ...item, value: Math.round(item.value * profile.factor) })),
    [profile.factor],
  );

  const filtered = useMemo(() => companyTransactions.filter((item) => {
  ...
  }), [activeMetric, companyTransactions, method, query, sort, status]);

  const sampleTotal = profile.total;
  const cash = profile.cash;
  const cards = profile.cards;
  const pending = companyTransactions.filter((item) => item.status !== "Conciliado").reduce((sum, item) => sum + item.amount, 0);
  const maxRevenue = Math.max(...dailyRevenue.map((item) => item.value));
  const monthlyTotal = profile.months.reduce((sum, value) => sum + value, 0);
  const monthlyShares = profile.months.map((value) => Math.round((value / monthlyTotal) * 100));
  const pieBackground = `conic-gradient(#BFD2FF 0 ${monthlyShares[0]}%, #2EA8FF ${monthlyShares[0]}% ${monthlyShares[0] + monthlyShares[1]}%, #3150D8 ${monthlyShares[0] + monthlyShares[1]}% 100%)`;
  ...
  return (
    <AppShell title="Recaudación" description="Control financiero, cierres y conciliación">
      <div className="space-y-5">
        ...
        <section className="grid gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2 xl:grid-cols-[1.2fr_1fr_1fr_auto] xl:items-end">
          <label className="text-xs font-semibold text-slate-600"><span className="mb-1.5 block">Empresa</span><select value={company} onChange={(event) => { setCompany(event.target.value); setActiveMetric("total"); }} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-[#3150D8]">{Object.keys(companyProfiles).map((item) => <option key={item}>{item}</option>)}</select></label>
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            { id: "cash", label: "Efectivo", value: money(cash), description: `${sampleTotal ? Math.round((cash / sampleTotal) * 100) : 0}% de la recaudación`, icon: Banknote, color: "text-emerald-700" },
            { id: "cards", label: "Tarjetas", value: money(cards), description: `${sampleTotal ? Math.round((cards / sampleTotal) * 100) : 0}% de la recaudación`, icon: CreditCard, color: "text-sky-700" },
            { id: "pending", label: "Por revisar", value: money(pending), description: `${companyTransactions.filter((item) => item.status !== "Conciliado").length} movimientos`, icon: CircleAlert, color: "text-amber-700" },
        ...
        <div className="grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
          <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="grid gap-6 border-t border-slate-200 bg-slate-50 p-5 lg:grid-cols-[auto_0.8fr_1.2fr] lg:items-center">
              <div className="relative mx-auto h-52 w-52 rounded-full shadow-sm" style={{ background: pieBackground }}>
                <div className="absolute inset-9 grid place-items-center rounded-full bg-white text-center shadow-sm">
                  <span><b className="block text-sm text-[#041E42]">{money(sampleTotal)}</b><small className="text-[9px] text-slate-500">Julio 2026</small></span>
                </div>
              </div>
              <div>
                <h3 className="font-bold text-[#041E42]">Participación mensual</h3>
                <p className="mt-1 text-xs text-slate-500">{company === "Todas" ? "Todas las empresas" : company} · últimos tres meses</p>
                <div className="mt-4 space-y-2 text-xs">
                  {[
                    ["Mayo 2026", profile.months[0], monthlyShares[0], "bg-[#BFD2FF]"],
                    ["Junio 2026", profile.months[1], monthlyShares[1], "bg-[#2EA8FF]"],
                    ["Julio 2026", profile.months[2], monthlyShares[2], "bg-[#3150D8]"],
                  ].map(([label, value, share, color]) => (
                    <div key={label} className="grid grid-cols-[12px_1fr_auto_auto] items-center gap-2">
                      <i className={`h-3 w-3 rounded-full ${color}`} />
                      <span className="font-semibold text-slate-600">{label}</span>
                      <span className="tabular-nums text-slate-500">{money(value)}</span>
                      <strong className="w-8 text-right text-[#041E42]">{share}%</strong>
                    </div>
                  ))}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="flex items-center justify-between gap-3">
                  <div><h3 className="font-bold text-[#041E42]">Evolución mensual</h3><p className="mt-1 text-xs text-slate-500">Comparación de recaudación</p></div>
                  <TrendingUp className="h-5 w-5 text-[#3150D8]" />
                </div>
                <div className="mt-5 flex h-40 items-end justify-around gap-4 border-b border-slate-200 px-2">
                  {[
                    ["Mayo", profile.months[0], "bg-[#BFD2FF]"],
                    ["Junio", profile.months[1], "bg-[#2EA8FF]"],
                    ["Julio", profile.months[2], "bg-[#3150D8]"],
                  ].map(([label, value, color]) => (
                    <div key={label} className="flex h-full min-w-0 flex-1 flex-col justify-end gap-2 text-center">
                      <span className="text-[9px] font-bold text-slate-500">{money(value)}</span>
                      <span className={`mx-auto block w-full max-w-20 rounded-t-xl ${color}`} style={{ height: `${Math.max((value / Math.max(...profile.months)) * 82, 8)}%` }} />
                      <span className="pb-2 text-[10px] font-semibold text-slate-600">{label}</span>
                    </div>
                  ))}
                </div>
                <p className="mt-3 text-center text-[10px] text-slate-500">Valores demostrativos de la empresa seleccionada.</p>
              </div>
            </div>
          </section>
          ...
          <section id="medios-de-pago" className="scroll-mt-24 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
              <div className="flex items-center gap-3"><WalletCards className="h-5 w-5 text-[#3150D8]" /><div><h2 className="font-bold text-[#041E42]">Medios de pago</h2><p className="text-xs text-slate-500">Integraciones disponibles</p></div></div>
              <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold uppercase text-amber-700">2 pendientes</span>
            </div>
            <div className="space-y-3 p-4">
              {[
                { id: "transbank", name: "Transbank", description: "POS físico y servicios de pago Transbank." },
                { id: "tuu", name: "TUU", description: "Plataforma de pagos y terminales TUU." },
              ].map((provider) => (
                <article key={provider.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="font-bold text-[#041E42]">{provider.name}</h3>
                      <p className="mt-1 text-xs text-slate-500">{provider.description}</p>
                    </div>
                    <span className="whitespace-nowrap rounded-full bg-slate-200 px-2.5 py-1 text-[10px] font-bold uppercase text-slate-600">No configurado</span>
                  </div>
                  <dl className="mt-3 grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-xl bg-white p-2.5"><dt className="text-slate-400">Comercio</dt><dd className="mt-1 font-semibold text-slate-600">Por definir</dd></div>
                    <div className="rounded-xl bg-white p-2.5"><dt className="text-slate-400">Ambiente</dt><dd className="mt-1 font-semibold text-slate-600">Sin configurar</dd></div>
                    <div className="rounded-xl bg-white p-2.5"><dt className="text-slate-400">Estacionamientos</dt><dd className="mt-1 font-semibold text-slate-600">0 asignados</dd></div>
                    <div className="rounded-xl bg-white p-2.5"><dt className="text-slate-400">Terminales</dt><dd className="mt-1 font-semibold text-slate-600">0 registrados</dd></div>
                  </dl>
                  <button type="button" disabled className="mt-3 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-400">
                    Configuración pendiente
                  </button>
                </article>
              ))}
              <p className="px-1 text-[11px] leading-5 text-slate-500">Se requerirán el identificador de comercio, tipo de integración, ambiente, estacionamientos, terminales y credenciales técnicas. Las claves se almacenarán cifradas y no serán visibles.</p>
            </div>
          </section>
        </div>
