...
import { ArrowLeft, Building2, CalendarClock, CarFront, FileText, KeyRound, Mail, Phone, ShieldCheck, UserRound } from "lucide-react";
...
export default async function AbonadoDetallePage({ params }) {
  ...
  const abonado = getAbonadoById(resolvedParams.id);
  ...
  const iniciales = abonado.nombre
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
    .map((chunk) => chunk[0])
    ...
    <AppShell title="Abonados" description="Detalle individual del abonado">
      <div className="space-y-6">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex min-w-0 items-start gap-4">
              <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[#EEF4FF] text-xl font-semibold text-[#3150D8]">
                {iniciales || "AB"}
              </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
          <SectionCard title="Información personal" description="Datos base disponibles del abonado dentro del entorno demostrativo.">
            ...
            <div className="mt-4 space-y-3">
              {vehiculos.length > 0 ? (
                ...
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-600">
                  Este abonado no tiene vehículos registrados.
                </div>
              )}
            </div>
          </SectionCard>
          ...
          <SectionCard title="Credenciales" description="Credenciales y medios de acceso asociados al abonado.">
            <div className="space-y-3">
              {credenciales.length > 0 ? (
                ...
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-600">
                  Este abonado no tiene credenciales registradas.
                </div>
              )}
            </div>
          </SectionCard>
        ...
        <SectionCard title="Permisos de acceso" description="Configuración actual de accesos y cobertura horaria.">
          <div className="grid gap-4 md:grid-cols-2">
            {permisos.length > 0 ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-600">
                Este abonado no tiene permisos registrados.
              </div>
            )}
          </div>
        </SectionCard>
