...
export default function DataEntryPage() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#ECEFF1] text-[#263238]">
    {posMenuOpen ? <button type="button" aria-label="Cerrar menú" onClick={() => setPosMenuOpen(false)} className="fixed inset-0 z-20 bg-[#041E42]/45 lg:hidden" /> : null}
    <aside className={`${posMenuOpen ? "translate-x-0 lg:w-64" : "-translate-x-full lg:w-20 lg:translate-x-0"} fixed inset-y-0 left-0 z-30 flex w-[min(18rem,calc(100vw-2rem))] flex-col bg-[#455A64] p-3 text-white shadow-2xl transition-all duration-200`}>
    ...
    <main className={`min-h-screen min-w-0 bg-[#ECEFF1] transition-all ${posMenuOpen ? "lg:ml-64" : "lg:ml-20"}`}>
    <header className="flex min-h-16 items-center justify-between gap-2 border-b border-[#CFD8DC] bg-[#F8F9FA] px-3 shadow-sm sm:px-5"><div className="flex min-w-0 items-center gap-2 sm:gap-3"><button type="button" onClick={() => setPosMenuOpen(true)} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-[#CFD8DC] bg-white text-[#455A64] lg:hidden" title="Abrir menú"><Menu className="h-5 w-5" /></button><button type="button" onClick={() => router.push("/")} className="hidden h-9 items-center gap-1.5 rounded-lg border border-[#CFD8DC] bg-[#ECEFF1] px-3 text-xs font-bold text-[#455A64] transition hover:bg-[#CFD8DC] sm:inline-flex" title="Volver"><ArrowLeft className="h-4 w-4" />Volver</button><div className="min-w-0"><p className="truncate text-base font-black text-[#263238] sm:text-xl">Park<span className="text-[#455A64]">Facil</span> Terminal</p><p className="truncate text-[9px] font-bold uppercase tracking-[0.12em] text-[#78909C] sm:text-[10px] sm:tracking-[0.16em]">Inicio de operación</p></div></div><div className="flex shrink-0 items-center gap-2"><div className="hidden text-right sm:block"><p className="text-xs font-bold">{actor?.name || "Cargando..."}</p><p className="text-[10px] text-[#455A64]">{roleLabel(actor?.role)}</p></div><button onClick={logout} className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 bg-white text-rose-600" title="Cerrar sesión"><LogOut className="h-4 w-4" /></button></div></header>
    <div className="mx-auto max-w-6xl p-3 sm:p-4">
      <section className="grid grid-cols-2 gap-2 sm:gap-2.5 lg:grid-cols-4">{actions.map((action) => { const Icon = action.icon; return <button key={action.key} onClick={() => selectView(action.key)} className={`${action.color} min-h-20 min-w-0 rounded-2xl p-3 text-left text-white shadow-md transition active:scale-[.98] sm:min-h-24 sm:p-4`}><Icon className="h-6 w-6 sm:h-7 sm:w-7"/><span className="mt-1.5 block truncate text-sm font-black sm:text-lg">{action.label}</span><span className="block truncate text-[10px] text-white/80 sm:text-xs">{action.detail}</span></button>; })}</section>
      <section className="mt-3 rounded-3xl border border-[#CFD8DC] bg-[#F8F9FA] p-4 shadow-lg sm:p-5">
        <div className="mb-4 flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[10px] font-black uppercase tracking-[0.16em] text-[#455A64]">{parking?.company_name || "ParkFacil"}</p><h1 className="mt-0.5 break-words text-lg font-black text-[#263238] sm:text-xl">{view === "HOME" ? (parking?.name || "Estacionamiento en operación") : actions.find((item) => item.key === view)?.label}</h1><p className="break-words text-xs font-bold text-[#78909C]">{view === "HOME" ? `Parking en operación${parking?.code ? ` · ${parking.code}` : ""}` : (parking?.name || "Estacionamiento asignado")}</p></div>{view === "PARKED" ? <div className="flex shrink-0 items-center gap-2"><button onClick={() => selectView("HOME")} className="hidden h-9 items-center gap-1.5 rounded-lg bg-[#ECEFF1] px-3 text-xs font-bold text-[#263238] sm:inline-flex"><ArrowLeft className="h-4 w-4" />Volver</button><button onClick={printPendingVehicles} title="Imprimir listado de vehículos pendientes" className="grid h-9 w-9 place-items-center rounded-lg border border-[#CFD8DC] bg-white text-[#455A64]"><Printer className="h-4 w-4" /></button></div> : view !== "HOME" ? <button onClick={() => selectView("HOME")} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#ECEFF1] text-[#263238]"><X className="h-4 w-4" /></button> : null}</div>
        ...
        {view === "ENTRY" ? <form onSubmit={registerEntry} className="mx-auto max-w-2xl space-y-6"><div className="rounded-2xl border border-[#CFD8DC] bg-[#ECEFF1] p-4"><p className="text-xs font-black uppercase tracking-wide text-[#455A64]">Estacionamiento asignado</p><p className="mt-1 break-words text-base font-black text-[#263238] sm:text-lg">{parking?.name || "Cargando asignación..."}</p></div><fieldset><legend className="text-sm font-black text-[#263238]">PATENTE</legend><div className="mt-2 flex min-w-0 items-center justify-center gap-2 sm:gap-3"><input ref={prefixRef} required maxLength={4} value={prefix} onChange={(event) => { const value=event.target.value.toUpperCase().replace(/[^A-Z0-9]/g,"");setPrefix(value);if(value.length===4)suffixRef.current?.focus(); }} placeholder="CXPY" className="min-h-20 min-w-0 flex-1 rounded-2xl border-2 border-slate-200 bg-slate-50 text-center text-2xl font-black uppercase tracking-[.08em] text-[#263238] outline-none focus:border-[#455A64] sm:min-h-24 sm:max-w-48 sm:rounded-3xl sm:text-4xl sm:tracking-[.15em]"/><span className="shrink-0 text-2xl font-black text-[#263238] sm:text-4xl">-</span><input ref={suffixRef} required maxLength={2} value={suffix} onChange={(event) => setSuffix(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g,""))} placeholder="93" className="min-h-20 w-20 min-w-0 shrink-0 rounded-2xl border-2 border-slate-200 bg-slate-50 text-center text-2xl font-black uppercase tracking-[.08em] text-[#263238] outline-none focus:border-[#455A64] sm:min-h-24 sm:w-28 sm:rounded-3xl sm:text-4xl sm:tracking-[.15em]"/></div></fieldset><button disabled={busy || !parking} className="flex min-h-16 w-full items-center justify-center gap-2 rounded-3xl bg-emerald-600 px-3 text-base font-black text-white shadow-lg disabled:opacity-50 sm:min-h-18 sm:gap-3 sm:text-xl">{busy?<LoaderCircle className="h-6 w-6 animate-spin"/>:<Printer className="h-6 w-6 shrink-0"/>}<span>GUARDAR E IMPRIMIR INGRESO</span></button></form> : null}
        ...
        {view === "PARKED" ? <div className="overflow-x-auto rounded-2xl border border-[#CFD8DC]"><div className="min-w-[34rem]"><div className="grid grid-cols-[1fr_1.2fr_.8fr] bg-[#263238] px-4 py-3 text-xs font-black uppercase text-white"><span>Patente</span><span>Ingreso</span><span>Operador</span></div>{stays.length?stays.map((stay)=><button key={stay.id} onClick={()=>{setSelectedStayId(stay.id);void calculateExit({stayId:stay.id});}} className="grid w-full grid-cols-[1fr_1.2fr_.8fr] border-t border-[#ECEFF1] px-4 py-4 text-left text-[#263238] hover:bg-[#ECEFF1]"><b>{stay.license_plate}</b><span className="text-sm">{dateTime(stay.entry_at)}</span><span className="truncate text-sm">{stay.entry_operator_name}</span></button>):<p className="p-8 text-center text-[#607D8B]">No hay vehículos dentro del parking.</p>}</div></div> : null}
        {view === "QR" ? <div className="mx-auto max-w-2xl space-y-5"><p className="rounded-2xl bg-amber-50 p-3 text-sm font-semibold text-amber-800">Para aplicar un cupón, selecciona primero el vehículo en Salida y luego lee su QR.</p><button onClick={startScanner} className="flex min-h-16 w-full items-center justify-center gap-2 rounded-3xl bg-amber-500 px-3 text-base font-black text-[#263238] sm:min-h-20 sm:gap-3 sm:text-xl"><Camera className="h-7 w-7 shrink-0"/>LEER QR CON CÁMARA</button>{cameraOpen?<div className="relative overflow-hidden rounded-3xl bg-black"><video ref={videoRef} className="aspect-video w-full object-cover"/><button onClick={stopScanner} className="absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-full bg-white"><X className="h-5 w-5"/></button></div>:null}<div className="flex flex-col gap-2 sm:flex-row"><input value={qrToken} onChange={(event)=>setQrToken(event.target.value.trim())} placeholder="Código QR manual" className="min-h-14 min-w-0 flex-1 rounded-2xl border-2 border-slate-200 bg-white px-4 font-mono text-[#263238] outline-none focus:border-[#455A64]"/><button onClick={()=>processQr(qrToken)} className="min-h-12 rounded-2xl bg-[#455A64] px-5 font-black text-white">LEER QR</button></div></div> : null}
