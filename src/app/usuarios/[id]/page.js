...
import { ArrowLeft, Briefcase, Mail, Phone, Users, Building2, KeyRound, History, Clock3, AlertTriangle } from "lucide-react";
...
import { getUsuarioById, getEmpresaAsociada, getEstacionamientosAsociados, getPerfilLabel } from "@/data/usuarios.mjs";
...
export default function UsuarioDetallePage({ params }) {
  const usuario = getUsuarioById(params.id);
  ...
  const estacionamientos = getEstacionamientosAsociados(usuario);
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-600">
                {estacionamientos.length > 0 ? estacionamientos.map((item) => <li key={item.id}>• {item.nombre}</li>) : <li>• Sin estacionamientos asignados</li>}
              </ul>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
