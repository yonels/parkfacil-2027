...
export async function POST(request) {
  ...
  if (!actor.isAdmin && !actor.isSupervisor) return NextResponse.json({ error: "No tienes permisos para generar cupones." }, { status: 403 });
  const input = await request.json(); const quantity = Math.max(1, Math.min(100, Math.floor(Number(input.quantity) || 1)));
  const db = getSupabaseAdminClient(); let sourceQuery = db.from("coupons").select("*").eq("id", input.sourceId);
  if (!actor.isPlatformAdmin) sourceQuery = sourceQuery.eq("company_id", actor.companyId);
  const { data: source } = await sourceQuery.maybeSingle(); if (!source) return NextResponse.json({ error: "Cupón base no encontrado." }, { status: 404 });
  let created = [];
  if (quantity > 1) {
    const rows = Array.from({ length: quantity - 1 }, () => ({ company_id: source.company_id, merchant_id: source.merchant_id, redeeming_merchant_id: source.redeeming_merchant_id, code: `PF-${randomBytes(4).toString("hex").toUpperCase()}`, name: source.name, benefit_type: source.benefit_type, benefit_value: source.benefit_value, status: source.status, valid_from: source.valid_from, expires_at: source.expires_at, created_by: actor.id }));
    const result = await db.from("coupons").insert(rows).select(selection); if (result.error) return NextResponse.json({ error: "No fue posible generar el lote de cupones." }, { status: 503 });
    created = result.data || [];
  }
  const { data: original } = await db.from("coupons").select(selection).eq("id", source.id).single();
  return NextResponse.json({ data: [original, ...created].map(mapCoupon) }, { status: 201 });
}
