...
create policy responsables_read on public.abonado_responsables for select to authenticated using (exists(select 1 from public.abonados a where a.responsable_id=abonado_responsables.id));
